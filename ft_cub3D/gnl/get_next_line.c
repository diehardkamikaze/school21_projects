...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/24 19:26:42 by mchau             #+#    #+#             */
/*   Updated: 2021/03/18 18:46:16 by mchau            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		get_next_line(int fd, char **line)
{
	...
	o.divider = 0;
	o.buffer = 0;
	if (remainder && (o.buffer = remainder))
	{
		remainder = 0;
	...
	while (o.i || (o.i = read(fd, o.buffer, BUFFER_SIZE)))
		if ((time = one_iter(&o, &remainder, line)) != -2)
			return (time);
	if (!(make_result_array(line, &o.lst, o.line_len)))
		return (selected_free(o.buffer, remainder, o.lst));
	selected_free(o.buffer, remainder, o.lst);
	return (0);
}
